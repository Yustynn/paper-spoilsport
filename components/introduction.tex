...
% \todo{this sentence immediately makes the contribution very weak. Can we rephrase that we propose a method of discovery instead?} 
\revs{In this work, we propose a method to discover details and examples of ERC-20 rug pull mechanics by testing against generic ERC-20 user perspectives.}
% In this work, we investigate whether testing against generically-specified ERC-20 user perspectives 
% can recover and enrich a dataset of known rug pulls.
...
Existing fuzzers typically either
    rely on manual deployment scripts~\cite{foundry_fuzz,echidna},
    start fuzzing immediately after deployment~\cite{sfuzz_icse20,contractfuzzer_2018,echidna},
    fork full, potentially complex on-chain state~\cite{echidna,medusa,ityfuzz_issta23}
    or else search for states that are well-positioned for discovering security vulnerabilities~\cite{smartian_ase21,ConFuzzius_eurosp21,ityfuzz_issta23}, rather than usage-realism.
We address this by leveraging our fuzzer and role/goal semantics in order to specify state characteristics of such usage-realism and solving for a reachable 
state prior to beginning our goal analysis.
