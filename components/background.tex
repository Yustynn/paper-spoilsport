...
There is a wide range of techniques to guide this mutation, including use of runtime feedback such as coverage and prior static analysis such as data dependency.
...
\spoilsport is primarily differentiated by its target task - rather than seeking to find security vulnerabilities, it finds violations of specified user goals while also annotating the roles of transactors in the violating transaction sequences.
...
Usage-realism of the starting state is particularly important as \spoilsport is explicitly aimed at unearthing motivation dynamics of users in the wild.
...
\begin{enumerate}
    ...
    \item \textit{Find interesting states.}
        Another approach is to attempt to find interesting states~\cite{smartian_ase21,ConFuzzius_eurosp21,ityfuzz_issta23}.
...
%We propose the use of the same role/goal semantics which we use to specify user perspectives in order to declaratively specify characteristics of a valid starting state.

We declaratively specify characteristics of a valid starting state via the same role/goal semantics that capture user perspectives. % in order to declaratively specify characteristics of a valid starting state.
...
This is a reasonable intermediate state between full replay of transactions and starting from deployment, as it captures the core characteristics 
of usage-realism: that there are tokens distributed among users and that at least one user is liquid.
