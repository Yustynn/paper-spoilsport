...
\revs{
    The aim of \textit{Static Analysis} is to derive information that can reduce the search space for the fuzzer.
...
\textit{Address1} then calls a misleading \texttt{transferFrom()} function, which succeeded because it passes the two checks of the caller being whitelisted and transferring to the \owner.
