...
This is done through using the function summaries to find appropriate candidates that may meet our target charactestics, and then attempting to run those.

Our found state initialization sequence is shown in \autoref{ss:fig:rq1-mrmr-flow}.
...
The \owner then uses the \texttt{\_mints()} function to simulateneously whitelist \textit{Address4}, allowing it to transfer tokens, and overwrite \textit{Address4}'s balances with a large amount of tokens.
