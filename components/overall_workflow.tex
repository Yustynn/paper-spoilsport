...
Similar to \approach{}, \spoilsport takes the solidity source code of the target contract and the role/goal specification as input.
...
As final output, it delivers labeled goal violations complete with the same motivation analysis presented in \autoref{ch:goalseeker}.

The pipeline consists of four processing stages:
\begin{enumerate}
    ...
    \item \textit{Motivation Analysis} annotates the found violations using the methodology from \autoref{ch:goalseeker}.
\end{enumerate}

\subsection{Static Analysis}
Here, we generate function-level summaries which are more lightweight than in \approach{}.
Rather than extract full path conditions and state variable transformations, we retrieve the following:
\begin{enumerate}
...
Similar to \approach{}, we parse the goal specification to retrieve relevant state variables.
...
\subsection{Motivation Analysis}
We implement the same motivation analysis detailed in \autoref{ch:goalseeker}.
The purpose of this is to capture the effects on transactors' goals in a goal violating transaction sequence, as well as provide a single metric to communicate how well motivated the overall sequence was in terms of these goals.

We first label each transaction as the maximum of this ordering on the set of effects for its transactor's goals: \textit{goal-conforming} > \textit{goal-violating} > \textit{goal-neutral}.
For instance, if a transaction conforms to one of the transactor's goals while violating another, we label it as \textit{goal-conforming} in order to signal that there is some motivation for the transactor to make the transaction.

We then label the overall transaction sequence as either \textit{Motivated} (all transactions are \textit{goal-conforming} or \textit{goal-neutral}), \textit{Anti-Motivated} (all transactions are \textit{goal-violating} or \textit{goal-neutral}), \textit{Mixed Motivation} (there are both \textit{goal-conforming} and \textit{goal-violating} transactions in the sequence) or \textit{Motivation-Neutral} (all transactions are \textit{goal-neutral}).


\subsection{Specification of Roles and Goals}
We use the \approach{} framework to specify user roles and goals, keeping them largely similar to the original work.

\spoilsport takes user-supplied specification of roles and goals in the \approach{} framework.
These goals are used in place of bug or vulnerability test oracles.
The \approach{} framework allows for specifications of binary goals (\textit{does something happen?}), optimization goals (\textit{can I increase this value?}) and call capability goals (\textit{I want to be able to call this function}).
As discussed in \autoref{ch:goalseeker}, goal violations differ from traditional bugs and vulnerabilities in that they are independent of the developer's intention.
...
We largely reuse the specification developed in \autoref{ch:goalseeker}.
This is to demonstrate that no special tailoring is required for this to find useful rug pull mechanics.
We restate this specification below.
...
We depart from the original specification by making a single improvement which improves the intention of the general specification rather than targeting rug pulls in particular.
We exclude goal violations where a user reduces their own holdings (naively violating \maxowned{}), specified through the constraint \texttt{UserCaller != UserAddress}.
This is in keeping the principle behind \liquidity{}, which is that end-users should have control over their funds.
This allows us to remove the \texttt{transfer()} function filter in the original specification, which was introduced to exclude trivial cases of a \holder violating their own \maxowned goal.
However, we maintain the original filter of the \texttt{transferFrom()} function violating the \maxowned{} goal.
The \texttt{transferFrom()} function is typically used by a user who has been approved by the \holder to spend their funds.
Due to the expectedness of this ability, this was excluded in the original specification as well.

During our comparison with \approach{}, we suspend this improvement.
This is so that we may have an accurate comparison across the two solvers.


In keeping with \autoref{ch:goalseeker}, we specify the following goal for the \textit{Contract Owner} role:
\begin{enumerate}
    \item \textit{\rev{\maxsupply}}: $\max$ \texttt{totalSupply}.
        ...
        This captures the intuition that creating more tokens will increase the market capitalization of the project (though this is relationship is non-linear and subject to market dynamics).
\end{enumerate}
...
This is done using the \approach{} framework, in 27 lines of Python code.
...
As these are characteristics over groups of users, we extend the \approach{} framework by adding quantifiers over goals.
