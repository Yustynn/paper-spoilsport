...
We present the overall workflow of \spoilsport in \autoref{fig:spoilsport-architecture}.
...
%These loops are heavily similar, with 
They are similar and hence, for brevity, we only show 
the pseudocode of the \textit{Goal Violation Search} loop found in \autoref{ss:code:fuzz}.
...
%If a seed fails to execute, we extend it by prepending candidate enabling transactions.
%These candidates are functions which modify the variables which the failing transaction's control-flow depends on.
This uses a similar insight to a prior work~\cite{ethploit}, constructing a transaction sequence from back to front.
...
For instance, a transaction sequence of length five which has \revs{satisfied} three goals would get precedence over a transaction sequence of length four which has \revs{satisfied} two goals.
