...
However, this is partially mitigated by our analysis in \textbf{RQ3}, which finds 964 non-\textit{Transaction Limitation} rug pull mechanics.
...
\textbf{Limitations of the goal violation discovery task:} 
    ...
    There are legitimate purposes for features underlying some goal violations which are not fraudulent, such as the ability to \textit{blacklist} suspicious users for regulatory compliance.
    However, these may still be abusable (e.g., by blacklisting non-suspicious users).
    ...
    For instance, rug pull contracts often rely on a router smart contract for integration with decentralized exchanges, which motivates a router role.
    Therefore the specification may be incomplete with respect to particular contracts.

\smallskip
\noindent
\textbf{Limitations of the \spoilsport \revs{fuzzer}:} 
...
Additionally, our \revs{fuzzer} does not support multi-contract analysis.
